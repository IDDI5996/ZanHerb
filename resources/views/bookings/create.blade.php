    ...
    <div class="container mx-auto px-4 max-w-4xl">
        ...
        <div class="bg-white rounded-xl shadow-lg overflow-hidden">
            <form action="{{ route('bookings.store') }}" method="POST" class="p-6 md:p-8">
                ...
                <!-- Hidden field for product ID from URL -->
                @if(request()->has('product_id'))
                    <input type="hidden" name="product_id_from_url" value="{{ request('product_id') }}">
                @endif
                            ...
                            <select id="product_id" name="product_id"
                                ...
                                @foreach($products as $product)
                                    <option value="{{ $product->id }}" 
                                        @if(old('product_id', request('product_id')) == $product->id) selected @endif
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Initialize with product from URL if available
